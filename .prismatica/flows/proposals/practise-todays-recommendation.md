...
proposalType: update
reason: The approved Delta
  (.prismatica/deltas/20260830-while-the-block-is-genuinely-running-and-899c.md)
  describes the new step-4 behaviour; this proposal carries that same wording
  into the flow's own canonical truth so the Flow record itself, not only the
  Delta, reflects it.
proposedBy: agent
createdAt: 2026-08-30T13:03:00.786Z
status: works
presentation:
  ...
  steps:
    ...
    - actor: The musician
      ...
      shows: The elapsed clock, and the item's notes and current problem on request.
        ...
        holds a screen wake lock so the clock stays readable without touching
        anything; pausing, finishing, discarding or navigating away releases it.
        Reaching the target visibly announces it once, with a growing overtime
...
**Reason:** The approved Delta (.prismatica/deltas/20260830-while-the-block-is-genuinely-running-and-899c.md) describes the new step-4 behaviour; this proposal carries that same wording into the flow's own canonical truth so the Flow record itself, not only the Delta, reflects it.
